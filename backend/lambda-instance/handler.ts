...
import { getMyPositions, getPositions, getOlpApr, hasPosition, getMyTradeData, getMyPositionHistory, getMyAddLiquidity, getUserAddLiquidity, getCopyTradePositionHistory, getOlpStatsDaily, getVolatilityScore, getMyAccountSummary } from './src/api'
...
export const connectTwitter = async (event) => {
  await loadEnv()
  const allowedOrigins = [process.env.MOBY_APP_URL];
  const origin = event.headers.origin;
  const isAllowedOrigin = allowedOrigins.includes(origin);
...
export const twitterCallback = async (event) => {
  ...
  return {
    ...
    headers: {
      ...
      'Access-Control-Allow-Methods': 'GET, POST, PUT, DELETE, OPTIONS',
      'Access-Control-Allow-Credentials': true,
      'Location': process.env.MOBY_APP_URL
    },
    body: ""
  }
}
