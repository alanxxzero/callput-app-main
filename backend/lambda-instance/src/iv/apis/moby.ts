import { InstrumentMarkData, InstrumentMarkDataRes } from '@moby/shared';
...
/**
 * Fetches mark IV and price data for instruments from the Moby API
 * @returns Promise with instrument mark data
 */
export const getInstrumentMarkDataFromMoby = async (): Promise<InstrumentMarkData> => {
  try {
    ...
    if (instrumentMarketDataRes.lastUpdatedAt < Date.now() - TIME_INVALID_THRESHOLD) {
      ...
      await sendMessage(
        `\`[Lambda][moby.ts]\` ${MESSAGE_TYPE.INSTRUMENT_MARK_DATA_NOT_UPDATED}`,
        LogLevel.CRITICAL,
        {
