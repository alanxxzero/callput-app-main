...
export const updateOptionsMarket = async (instrumentsToApply) => {
  ...
  const activeInstruments = uniq(
    instruments.active.map(removeCallPutPart)
  )

  const inactiveInstruments = uniq(
    instruments.inactive
      .map(removeCallPutPart)
  )
  ...
  for await (const instrumentNameList of chunkedActiveInstruments) {
    ...
    const optionInfoList = await OptionsMarket.getOptionsBatch(instrumentNameList.map((instrumentName) => getOptionKey(instrumentName)))
  ...
  for await (const [groupKey, options] of Object.entries(listToActivate)) {
    ...
    const txData = await OptionsMarket.addOptions.populateTransaction(
  ...
  for await (const [groupKey, options] of Object.entries(listToDeactivate)) {
    ...
    const txData = await OptionsMarket.removeOptions.populateTransaction(
