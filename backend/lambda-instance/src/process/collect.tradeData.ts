...
import { ChainNames, isBuyStrategy, isCallStrategy, parseOptionTokenId, UA_INDEX_TO_TICKER, UA_TICKER_TO_DECIMAL, UnderlyingAssetIndex } from "@moby/shared";
...
export const collectTradeData = async () => {
  ...
  try {
    ...
    const data = await fetchDataFromS3Gzip({
      ...
      Key: process.env.MOBY_DATA_TRADE_DATA_KEY + '.gz',
      initialData
    })
