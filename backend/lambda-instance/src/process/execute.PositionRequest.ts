...
import { calculateEstimatedSizeForOpenPosition, calculateOpenPositionFee, calculateRiskPremiumRate, calculateUnderlyingFutures, ChainNames, generateOptionTokenData, getInstrumentMarkDataFromMarket, getMainOptionName, getMainOptionStrikePrice, getMarkIvAndPriceByInstrument, getMarkIvAndPriceByOptionTokenId, getPairedOptionName, getPairedOptionStrikePrice, Greeks, isBuyStrategy, isCallStrategy, OptionDirection, OptionRiskPremiumInputData, parseOptionTokenId, QA_ADDRESS_TO_TICKER, QA_TICKER_TO_DECIMAL, SpotAssetIndexMap, UA_INDEX_TO_TICKER, UA_TICKER_TO_DECIMAL, UnderlyingAssetIndex } from "@moby/shared";
...
export const executePositionRequest = async () => {
  ...
  try {
    const { FastPriceFeed, ViewAggregator } = await initializeContracts();
    ...
    if (position_optionTokenIdsToExecute.length > 0) {
      const txData = await FastPriceFeed.setPricesAndRiskPremiumsWithBitsAndExecute.populateTransaction(
      ...
      const { isSuccess, receipt } = await safeTx(
        Keeper.POSITION_PROCESSOR,
        {
          to: txData.to,
          data: txData.data,
          value: "0"
...
const getPositionRequests = async (
  positionRequestInfo,
  olpUtilityRatio
) => {
  ...
  for (const [requestIndex, isOpen, account, optionTokenId, amountInOrSize, blockTime, status, sizeOutOrAmountOut, executionPrice, processBlockTime, path0, path1] of positionRequestInfo) {
...
const fillPricesToRequests = async (
  ...
  const [instrumentMarkData, { data: futuresAssetIndexMap }, { data: spotAssetIndexMap }, riskFreeRates, { data: volatilityScore }] = await Promise.all([
    getInstrumentMarkDataFromMoby(),
    getFuturesS3(),
    getSpotS3(),
    getRiskFreeRatesS3(),
    ...
    const logsToPush = {
      ...
      amountInToExecute: isOpen // for closeBuySell, it's 0
        ? new BigNumber(quoteAssetAmount) 
          .div(10 ** QA_TICKER_TO_DECIMAL[chainName][quoteAsset])
          .toNumber()
        : 0,
