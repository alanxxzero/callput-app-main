...
export const distributeReward = async () => {
  ...
  const { FeeDistributor, sRewardTracker, mRewardTracker, lRewardTracker } = await initializeContracts();
  ...
  const solpPendingRewards = await (FeeDistributor.pendingOLPRewards(CONTRACT_ADDRESSES[chainId].S_REWARD_DISTRIBUTOR));
  const molpPendingRewards = await (FeeDistributor.pendingOLPRewards(CONTRACT_ADDRESSES[chainId].M_REWARD_DISTRIBUTOR));
  const lolpPendingRewards = await (FeeDistributor.pendingOLPRewards(CONTRACT_ADDRESSES[chainId].L_REWARD_DISTRIBUTOR));

  try {
    const txData = await FeeDistributor.distributeOLPRewards.populateTransaction()
    await safeTx(
    ...
    const lastOlpRewardsDistribution = Number(await FeeDistributor.lastOlpRewardsDistribution())
    const newSOlpTokensPerInterval = new BigNumber(await sRewardTracker.tokensPerInterval()).div(10 ** 18).toString()
    const newMOlpTokensPerInterval = new BigNumber(await mRewardTracker.tokensPerInterval()).div(10 ** 18).toString()
    const newLOlpTokensPerInterval = new BigNumber(await lRewardTracker.tokensPerInterval()).div(10 ** 18).toString()
