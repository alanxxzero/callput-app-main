...
import { getInstrumentMarkDataFromMoby } from '../iv/apis/moby';
...
export const collectMarketData = async (count, isParallel) => {
  ...
  const [{ instruments }, { data: futuresAssetIndexMap }, { data: spotAssetIndexMap }, riskFreeRates, { data: volatilityScore }] = await Promise.all([
    getS3({
      ...
      Key: process.env.MOBY_DATA_INSTRUMENTS_KEY,
    }),
    getFuturesS3(),
    getSpotS3(),
    getRiskFreeRatesS3(),
  ...
  const instrumentMarkData = await getInstrumentMarkDataFromMoby();
  ...
  if (isParallel) {
    return putS3({
      Bucket: process.env.MOBY_DATA_BUCKET,
      Key: process.env.MOBY_DATA_MARKET_DATA_KEY,
  ...
  return putS3({
    Bucket: process.env.MOBY_DATA_BUCKET,
    Key: `${process.env.MOBY_DATA_MARKET_DAILY_KEY}-${dateISOString}.json`,
    Body: JSON.stringify(body),
    CacheControl: 'no-cache',
  })
}
...
const getOlpAssetAmounts = async (): Promise<any> => {
  ...
  if (chainId === 80094) {
    ASSETS = [CONTRACT_ADDRESSES[chainId].WBTC, CONTRACT_ADDRESSES[chainId].WETH, CONTRACT_ADDRESSES[chainId].USDC, CONTRACT_ADDRESSES[chainId].HONEY]
  } else {
    ASSETS = [CONTRACT_ADDRESSES[chainId].WBTC, CONTRACT_ADDRESSES[chainId].WETH, CONTRACT_ADDRESSES[chainId].USDC]
  }
  ...
  if (chainId === 80094) {
    return {
      sOlp: {
        ...
        usdc: {
          utilizedAmount: new BigNumber(sOlpAssetAmounts[6]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["USDC"]).toNumber(),
          availableAmount: new BigNumber(sOlpAssetAmounts[7]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["USDC"]).toNumber(),
          depositedAmount: new BigNumber(sOlpAssetAmounts[8]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["USDC"]).toNumber()
        },
        honey: {
          utilizedAmount: new BigNumber(sOlpAssetAmounts[9]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["HONEY"]).toNumber(),
          availableAmount: new BigNumber(sOlpAssetAmounts[10]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["HONEY"]).toNumber(),
          depositedAmount: new BigNumber(sOlpAssetAmounts[11]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["HONEY"]).toNumber()
        }
      },
      mOlp: {
        wbtc: {
        ...
        usdc: {
          utilizedAmount: new BigNumber(mOlpAssetAmounts[6]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["USDC"]).toNumber(),
          availableAmount: new BigNumber(mOlpAssetAmounts[7]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["USDC"]).toNumber(),
          depositedAmount: new BigNumber(mOlpAssetAmounts[8]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["USDC"]).toNumber()
        },
        honey: {
          utilizedAmount: new BigNumber(mOlpAssetAmounts[9]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["HONEY"]).toNumber(),
          availableAmount: new BigNumber(mOlpAssetAmounts[10]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["HONEY"]).toNumber(),
          depositedAmount: new BigNumber(mOlpAssetAmounts[11]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["HONEY"]).toNumber()
        }
      },
      lOlp: {
        wbtc: {
        ...
        usdc: {
          utilizedAmount: new BigNumber(lOlpAssetAmounts[6]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["USDC"]).toNumber(),
          availableAmount: new BigNumber(lOlpAssetAmounts[7]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["USDC"]).toNumber(),
          depositedAmount: new BigNumber(lOlpAssetAmounts[8]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["USDC"]).toNumber()
        },
        honey: {
          utilizedAmount: new BigNumber(lOlpAssetAmounts[9]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["HONEY"]).toNumber(),
          availableAmount: new BigNumber(lOlpAssetAmounts[10]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["HONEY"]).toNumber(),
          depositedAmount: new BigNumber(lOlpAssetAmounts[11]).dividedBy(10 ** ASSET_TICKER_TO_DECIMALS[chainId]["HONEY"]).toNumber()
        }
      }
    }
  }
