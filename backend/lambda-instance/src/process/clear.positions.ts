...
import { ChainNames, isSameAddress, UA_INDEX_TO_TICKER, UnderlyingAssetIndex } from '@moby/shared'
...
export const clearPositions = async () => {
  ...
  try {
    const s_positionQueryFilter = getPositionQueryFilter(CONTRACT_ADDRESSES[chainId].S_VAULT, PositionState.NOT_EXPIRED, SettleState.NOT_SETTLED, null)
    const m_positionQueryFilter = getPositionQueryFilter(CONTRACT_ADDRESSES[chainId].M_VAULT, PositionState.NOT_EXPIRED, SettleState.NOT_SETTLED, null)
    const l_positionQueryFilter = getPositionQueryFilter(CONTRACT_ADDRESSES[chainId].L_VAULT, PositionState.NOT_EXPIRED, SettleState.NOT_SETTLED, null)
    ...
    // BTC
    await clearWithChunk(CONTRACT_ADDRESSES[chainId].S_VAULT, AssetTicker.BTC, s_clearAvailableOptions[AssetTicker.BTC], Controller)
    await clearWithChunk(CONTRACT_ADDRESSES[chainId].M_VAULT, AssetTicker.BTC, m_clearAvailableOptions[AssetTicker.BTC], Controller)
    await clearWithChunk(CONTRACT_ADDRESSES[chainId].L_VAULT, AssetTicker.BTC, l_clearAvailableOptions[AssetTicker.BTC], Controller)

    // ETH
    await clearWithChunk(CONTRACT_ADDRESSES[chainId].S_VAULT, AssetTicker.ETH, s_clearAvailableOptions[AssetTicker.ETH], Controller)
    await clearWithChunk(CONTRACT_ADDRESSES[chainId].M_VAULT, AssetTicker.ETH, m_clearAvailableOptions[AssetTicker.ETH], Controller)
    await clearWithChunk(CONTRACT_ADDRESSES[chainId].L_VAULT, AssetTicker.ETH, l_clearAvailableOptions[AssetTicker.ETH], Controller)
...
const clearWithChunk = async (vault: string, ticker: AssetTicker, options: ClearAvailableOption[], Controller: Contract) => {
  ...
  for await (const _options of chunked) {
    ...
    const txData = await Controller.pluginClearPosition.populateTransaction(
      _options.map(option => option.optionTokenId),
      vault,
    )

    await safeTx(
      Keeper.CLEARING_HOUSE,
      {
        to: txData.to,
        data: txData.data,
        value: "0"
...
const getClearAvailableOptions = (targetVault, positions) => {
  ...
  return Object.entries(grouped).reduce((acc, [optionNames, options]: any) => {
