...
import { ChainNames, generateOptionTokenData, getMainOptionName, getPairedOptionName, isSpreadStrategy, parseOptionTokenId, Strategy, UA_INDEX_TO_DECIMAL, UnderlyingAssetIndex } from "@moby/shared";
...
export const notifyPositions = async () => {
  ...
  const isSuccess = await sendMessage("PositionHistory", LogLevel.INFO, {
    ...
    isTrade: true,
    ...(bigPositionExists && { tags: [SlackTag.MOBY] })
  });
