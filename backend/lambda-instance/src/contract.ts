...
const initializeContracts = async () => {
  ...
  const keeper_optionsMarket = new ethers.Wallet(process.env.KEEPER_OPTIONS_MARKET_KEY, provider)
  const keeper_positionProcessor = new ethers.Wallet(process.env.KEEPER_POSITION_PROCESSOR_KEY, provider)
  const keeper_settleOperator = new ethers.Wallet(process.env.KEEPER_SETTLE_OPERATOR_KEY, provider)
  const keeper_positionValueFeeder = new ethers.Wallet(process.env.KEEPER_POSITION_VALUE_FEEDER_KEY, provider)
  const keeper_positionValueFeeder_sub1 = new ethers.Wallet(process.env.KEEPER_POSITION_VALUE_FEEDER_SUB1_KEY, provider)
  const keeper_spotPriceFeeder = new ethers.Wallet(process.env.KEEPER_SPOT_PRICE_FEEDER_KEY, provider)
  const keeper_spotPriceFeeder_sub1 = new ethers.Wallet(process.env.KEEPER_SPOT_PRICE_FEEDER_SUB1_KEY, provider)
  const keeper_feeDistributor = new ethers.Wallet(process.env.KEEPER_FEE_DISTRIBUTOR_KEY, provider)
  const keeper_clearingHouse = new ethers.Wallet(process.env.KEEPER_CLEARING_HOUSE_KEY, provider)
  ...
  return {
    ...
    keeper_positionValueFeeder_sub1,
    keeper_spotPriceFeeder,
    keeper_spotPriceFeeder_sub1,
