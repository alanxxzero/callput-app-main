...
import { calculateRiskPremiumRate, calculateUnderlyingFutures, ChainNames, FEE_RATES, formatOptionTokenId, FuturesAssetIndexMap, generateOptionTokenData, getInstrumentMarkDataFromMarket, getMainOptionName, getMainOptionStrikePrice, getMarkIvAndPriceByInstrument, getMarkIvAndPriceByOptionTokenId, getPairedOptionName, getPairedOptionStrikePrice, OptionRiskPremiumInputData, SpotAssetIndexMap, TRADE_FEE_CALCULATION_LIMIT_RATE, UA_TICKER_TO_INDEX, UnderlyingAsset } from "@moby/shared"

export const getComboCollateralAmount = async (event) => {  
  const { items } = JSON.parse(event.body)
  const { data } = await getS3({ Bucket: process.env.MOBY_DATA_BUCKET, Key: process.env.MOBY_DATA_MARKET_DATA_KEY })
