...
export const updateDashboardData = async () => {
    try {
        const { DUNE_DATA_BUCKET, MOBY_DATA_OLP_DASHBOARD_KEY, MOBY_DATA_CLOUDFRONT_DISTRIBUTION_ID} = process.env;
        ...
        */
        ...
        await invalidateCloudfrontCache({
            DistributionId: MOBY_DATA_CLOUDFRONT_DISTRIBUTION_ID,
            InvalidationPaths: [`/${MOBY_DATA_OLP_DASHBOARD_KEY}`],
        });
