...
export const initializeRedis = async () => {
    const redis: Redis =
        prevRedis ||
        new Redis({
            host: process.env.MOBY_REDIS_HOST,
            port: 6379,
            password: process.env.MOBY_REDIS_PASSWORD,
    ...
    const redisGlobal: Redis =
        prevRedisGlobal ||
            new Redis({
                host: process.env.MOBY_REDIS_GLOBAL_HOST,
                port: 6379,
                password: process.env.MOBY_REDIS_GLOBAL_PASSWORD,
                connectTimeout: 10 * 1000, // 10 seconds
                retryStrategy: (times) => {
                    retryCount++;
...
export const getDailyRedis = async (dailyKey: string, isRedisGlobal: boolean = false): Promise<{ data: any, lastUpdatedAt: number }> => {
    ...
    const daily = await decompressData(await redis.getBuffer(dailyKey));
