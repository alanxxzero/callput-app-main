...
import { FuturesAssetIndexMapRes, SpotAssetIndexMapRes } from '@moby/shared';
...
export const shouldUpdateOlppv = (
  prevOlppv: Olppv, 
  currOlppv: Olppv,
  currentPositionKeysStart?: number,
  onChainAckPositionKeysStart?: number
): boolean => {
  // Check for position state mismatch first
  if (currentPositionKeysStart !== undefined && onChainAckPositionKeysStart !== undefined) {
    const positionStateMismatch = currentPositionKeysStart !== onChainAckPositionKeysStart;
    if (positionStateMismatch) {
      console.log('shouldUpdateOlppv: Position state mismatch detected');
      console.log('  currentPositionKeysStart:', currentPositionKeysStart);
      console.log('  onChainAckPositionKeysStart:', onChainAckPositionKeysStart);
      return true;
    }
  }
