...
export const feedSpot = async (assets: string[]) => {
  ...
  try {
    ...
    if (shouldUpdate) {
      const { SpotPriceFeed } = await initializeContracts();

      const txData = await SpotPriceFeed.feedSpotPrices.populateTransaction(
        assets.map((asset) => {
      ...
      await processFeedTx('feedSpot', async () => {
        return await safeTx(
          Keeper.SPOT_PRICE_FEEDER,
          {
            to: txData.to,
            data: txData.data,
            value: "0"
