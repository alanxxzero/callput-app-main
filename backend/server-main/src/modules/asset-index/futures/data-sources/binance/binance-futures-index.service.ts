...
import { BinanceFuturesIndexRes } from './binance-futures-index.interface';
...
import { NormalizedAssetType, NormalizedFuturesAsset } from '@moby/shared';

@Injectable()
export class BinanceFuturesIndexService extends BaseAssetIndexService<NormalizedFuturesAsset> {
  protected readonly logger = new Logger(BinanceFuturesIndexService.name);
  ...
  private readonly BINANCE_FUTURES_INDEX_URL = 'https://www.binance.com/fapi/v1/premiumIndex?symbol=';
  ...
  protected async fetchAndTransform(asset: NormalizedFuturesAsset, retryCount = 0): Promise<AssetIndexData> {
    try {
      ...
      const response = await axios.get<BinanceFuturesIndexRes>(this.BINANCE_FUTURES_INDEX_URL + symbol, {
      ...
      return {
        name: asset,
        price: parseFloat(result.markPrice),
  ...
  @Cron(CronExpression.EVERY_SECOND, {
    name: 'process-binance-futures-index',
    timeZone: 'UTC',
  })
  private async processBinanceFuturesIndex() {
    try {
      ...
      this.logger.error(
        'Failed to process binance futures index:',
  ...
  async onModuleInit() {
    ...
    await this.processBinanceFuturesIndex();
  }
}
