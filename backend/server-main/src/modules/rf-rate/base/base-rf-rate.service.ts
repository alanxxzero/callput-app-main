...
import { RiskFreeRateCollection, RiskFreeRateCollectionRes, UnderlyingAssetExpiryMapRes } from '@moby/shared';
export abstract class BaseRiskFreeRateService<T> implements OnModuleInit {
  ...
  protected refreshRiskFreeRateCollection(underlyingAsset: UnderlyingAsset) {
    const validExpiries = new Set(this.underlyingAssetExpiryMapRes.data[underlyingAsset]);
    const rateMap = this.riskFreeRateCollectionRes.data[underlyingAsset];
  ...
  protected updateRiskFreeRateCollection(
    ...
    this.underlyingAssetExpiryMapRes.data[underlyingAsset] = newExpiries;
    ...
    for (const [expiry, underlyingFutures] of newExpiryPrices) {
      ...
      this.riskFreeRateCollectionRes.data[underlyingAsset][expiry] = riskFreeRate;
