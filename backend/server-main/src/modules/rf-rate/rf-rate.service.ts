...
import { MultipleRiskFreeRateCollectionRes } from '@moby/shared';
...
export class RiskFreeRateService extends BaseRiskFreeRateService<any> {
  ...
  protected readonly sourceName = 'moby';
  ...
  private setRiskFreeRateCollection(underlyingAsset: UnderlyingAsset) {
    ...
    this.riskFreeRateCollectionRes.data[underlyingAsset] = {};

    Object.entries(rawRiskFreeRates).forEach(([expiry, rates]) => {
      const averageRate = rates.reduce((sum, rate) => sum + rate, 0) / rates.length;
      this.riskFreeRateCollectionRes.data[underlyingAsset][Number(expiry)] = averageRate;
    });
