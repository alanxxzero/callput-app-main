...
import { loadOlpApr, loadOlpMetrics } from './store/slices/AppSlice'
...
function App() {
  ...
  useEffect(() => {
    const initialAnnouncements: AnnouncementData[] = [
      ...
      //           name: SupportedChains.ARBITRUM_ONE,
      //           isBerachain: isBerachain(SupportedChains.ARBITRUM_ONE)
      //         })}>
      //           Arbitrum Mainnet
      //         </span>{' '}
      //         only. They will be supported on Berachain shortly.
      //       </p>
      //     </div>
      //   ),
      //   supports: [SupportedChains.BERACHAIN_MAINNET, SupportedChains.BERACHAIN_CARTIO],
      //   isCheckable: true,
      // }
    ];

    setAnnouncements(initialAnnouncements.filter(announcement => {
      const hideUntil = localStorage.getItem(`moby:announcementBar:${announcement.id}:hideUntil`);
          ...
          {isMobile
            ...
            : <>
                ...
                <Routes>
                  ...
                  <Route path="/earn" element={<Earn announcementsLen={announcements.length} />} />
