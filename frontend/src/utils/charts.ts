...
import { ChartData, ChartDataPoint, GenerateChartDataArg, Position } from "./types";
import { calculateBlackScholesProfit } from "@moby/shared";

// Chart Range 계산 공식
export const calculateChartRange = (positions: Position[]) => {
  const result = positions.reduce((acc, { strikePrice }) => { 
  ...
  // search boundary
  const underlyingMinPrice = Math.floor(result.underlyingMinPrice * 0.1 / 100) * 100;
  const underlyingMaxPrice = Math.ceil(result.underlyingMaxPrice * 10 / 100) * 100;
...
export const generateChartData = (
  ...
  for (let assetPrice = assetPriceMin; assetPrice <= assetPriceMax; assetPrice += tickInterval) {
    
    // 포지션이 하나일 때는 Profit은 단일 포지션에 대한 Profit
    // 포지션이 여러개일 때는 Profit은 각 포지션 Profit의 합
    let profit = 0

    chartDataRaw.forEach(({ isCall, strikePrice, orderPrice, qty, fromTime, expiry, volatility }, idx) => {
    ...
    if (idx === 0) {
      ...
      if (prevPathY > 0 && path[1] > 0) {
        ...
        if (prevPathY > 0) {
          ...
          const [adjustedPathX2, adjustedPathY2] = adjustFunction([path[0], path[1]])
          graphics.lineStyle(decreasingLineStyle)
        ...
        // - -> +
        if (path[1] > 0) {
          const [adjustedPathX1, adjustedPathY1] = adjustFunction([path[0], path[1]])
          graphics.lineStyle(decreasingLineStyle)
          graphics.lineTo(adjustedPathX1, adjustedPathY1)
