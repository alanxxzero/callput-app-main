...
import { multicall, simulateContract, waitForTransactionReceipt, writeContract } from "wagmi/actions";
...
import { IControllerAllowance, IPoolAllowance, IUserBalance } from "@/interfaces/interfaces.userSlice";
import { NewPosition, SettlePosition } from "@/interfaces/interfaces.positionSlice";
...
import { BerachainSpecificAddresses, ContractAddresses } from "@/networks/types";
import {
...
import { BN, NetworkQuoteAsset, UnderlyingAsset } from "@moby/shared";
import {
...
export function getContractAddress<T extends keyof (ContractAddresses & BerachainSpecificAddresses)>(
  contractName: T,
  chain: "Berachain Mainnet"
): `0x${string}`;


export function getContractAddress(contractName: string, chain: SupportedChains): `0x${string}` {
  const address =
    CONTRACT_ADDRESSES[chain][contractName as keyof (typeof CONTRACT_ADDRESSES)[typeof chain]];
...
const makeRequestTransaction = async ({
  ...
  try {
    ...
    const targetAddresses = Object.values(CONTRACT_ADDRESSES)
      .map((networkAddresses: any) => networkAddresses.POSITION_MANAGER?.toLowerCase())
      .filter((address) => address);

    const targetTopic = "0xd829e644f4628ea23643d25cad533269af97d942cfa01b92b5cea1769b99600a";
    ...
    if (transactionReceipt.status === "success") {
      ...
      const newPendingTxInfo: [string, NewPosition, SupportedChains] = [requestKey, txInfo, chain];
    ...
    const errorMessage = String(error.message).substring(0, MAX_SUBSTRING_LENGTH) + "...";
  ...
  try {
    ...
    if (transactionReceipt.status === "success") {
      addToastMessage({
        id: defaultId,
        type: "success",
        title: successMessage?.title || "Your transaction is processed successfully.",
...
const honeyContract = (chain: SupportedChains) => {
  return {
    address: getContractAddress("HONEY", chain as "Berachain Mainnet"),
    abi: ERC20Abi as Abi,
  };
};
...
export async function sendCreateOpenPosition<T extends SupportedChains>(
  ...
  const txInfo: NewPosition = {
    ...
    openedAmount: isBuy ? new BN(quoteAssetAmount).multipliedBy(10 ** QA_TICKER_TO_DECIMAL[chain][quoteAsset]).toString() : "0",
    openedCollateralToken: isBuy ? "" : QA_TICKER_TO_ADDRESS[chain][quoteAsset],
    openedCollateralAmount: isBuy ? "0" : new BN(quoteAssetAmount).multipliedBy(10 ** QA_TICKER_TO_DECIMAL[chain][quoteAsset]).toString(),
  ...
  const isEth = "ETH" in NetworkQuoteAsset[chain] && quoteAsset === NetworkQuoteAsset[chain].ETH;
  const path = getQuoteAssetPathForOpenPosition(chain, isBuy, isCall, underlyingAsset, length, quoteAsset);

  const args: IArgsForCreateOpenPosition = {
    userAddress: address,
    underlyingAssetIndex: getUnderlyingAssetIndexByTicker(chain, underlyingAsset),
    length: length,
    isBuys: isBuys,
    optionIds: optionIds,
    isCalls: isCalls,
...
function getQuoteAssetPathForOpenPosition<T extends SupportedChains>(
  ...
  if ("ETH" in NetworkQuoteAsset[chain] && quoteAsset === NetworkQuoteAsset[chain].ETH) {
    if (length === "1" && !isBuy && isCall) {
      // Vanilla Sell Call
      return [QA_TICKER_TO_ADDRESS[chain]["WETH"], UA_TICKER_TO_ADDRESS[chain][underlyingAsset]];
    }
    return [QA_TICKER_TO_ADDRESS[chain]["WETH"], QA_TICKER_TO_ADDRESS[chain]["USDC"]];
  }
  ...
  if (quoteAssetAddress.toLowerCase() === requiredQuoteAssetAddress.toLowerCase()) {
...
const getOptionIds = (length: string, mainOption: IOptionDetail, pairedOption: IOptionDetail) => {
  if (length === "1") {
...
export const writeCreateOpenPositionETH = async (
  ...
  const totalAmountIn = new BigNumber(args.amountIn).plus(EXECUTION_FEE).toString();
...
export const writeCreateClosePosition = async (
  ...
  try {
    const result = await makeRequestTransaction({
      ...
      args: [underlyingAssetIndex, optionTokenId, size, path, minAmountOut, minOutWhenSwap, withdrawETH],
      value: parseEther(EXECUTION_FEE),
      txInfo: txInfo,
      chain,
    });
...
export const writeMintAndStakeOlp = async (olpKey: OlpKey, args: any[], chain: SupportedChains) => {
...
export const writeUnstakeAndRedeemOlp = async (olpKey: OlpKey, args: any[], chain: SupportedChains) => {
  return await makeTransaction({
    ...rewardRouterContract(olpKey, chain),
...
export const writeHandleRewards = async (olpKey: OlpKey, chain: SupportedChains) => {
  return await makeTransaction({
    ...rewardRouterContract(olpKey, chain),
...
export const writeHandleAddReferral = async (address: string, chain: SupportedChains) => {
  return await makeTransaction({
...
export const writeApproveERC20 = async (tokenAddress: `0x${string}`, spender: string, amount: bigint) => {
  return await makeTransaction({
...
export const getAllowanceForController = async (
  ...
  try {
    ...
    if (isBerachain(chain)) {
      data.quoteToken.honey =
        (response[3].result &&
          new BigNumber(String(response[3].result))
            .dividedBy(10 ** QA_TICKER_TO_DECIMAL[chain]["HONEY"])
            .toString()) ||
        "0";
    }
  } catch (error) {
...
export const getAllowanceForPool = async (
  ...
  try {
    const contracts = [
      {
        ...wbtcContract(chain),
        functionName: "allowance",
        args: [address, getContractAddress("S_OLP_MANAGER", chain)],
      },
      {
        ...wbtcContract(chain),
        functionName: "allowance",
        args: [address, getContractAddress("M_OLP_MANAGER", chain)],
      },
      {
        ...wbtcContract(chain),
        functionName: "allowance",
        args: [address, getContractAddress("L_OLP_MANAGER", chain)],
      },
      {
        ...wethContract(chain),
        functionName: "allowance",
    ...
    data.sOlpManager.wbtc = (response[0].result && response[0].result.toString()) || "0";
    data.mOlpManager.wbtc = (response[1].result && response[1].result.toString()) || "0";
    data.lOlpManager.wbtc = (response[2].result && response[2].result.toString()) || "0";
    data.sOlpManager.weth = (response[3].result && response[3].result.toString()) || "0";
    data.mOlpManager.weth = (response[4].result && response[4].result.toString()) || "0";
    data.lOlpManager.weth = (response[5].result && response[5].result.toString()) || "0";
    data.sOlpManager.usdc = (response[6].result && response[6].result.toString()) || "0";
    data.mOlpManager.usdc = (response[7].result && response[7].result.toString()) || "0";
    data.lOlpManager.usdc = (response[8].result && response[8].result.toString()) || "0";

    if (isBerachain(chain)) {
      data.sOlpManager.honey = (response[9].result && response[9].result.toString()) || "0";
      data.mOlpManager.honey = (response[10].result && response[10].result.toString()) || "0";
      data.lOlpManager.honey = (response[11].result && response[11].result.toString()) || "0";
    }
  } catch (error) {
    console.error(error);
  }
...
export const getUserBalance = async (data: IUserBalance, address: string, chain: SupportedChains) => {
  try {
    ...
    data.olpToken.sOlp =
      (response[3].result && new BigNumber(String(response[3].result)).dividedBy(10 ** 18).toString()) || "0";
    data.olpToken.mOlp =
      (response[4].result && new BigNumber(String(response[4].result)).dividedBy(10 ** 18).toString()) || "0";
    data.olpToken.lOlp =
      (response[5].result && new BigNumber(String(response[5].result)).dividedBy(10 ** 18).toString()) || "0";

    data.claimableReward.sOlp =
      (response[6].result && new BigNumber(String(response[6].result)).dividedBy(10 ** 18).toString()) || "0";
    data.claimableReward.mOlp =
      (response[7].result && new BigNumber(String(response[7].result)).dividedBy(10 ** 18).toString()) || "0";
    data.claimableReward.lOlp =
      (response[8].result && new BigNumber(String(response[8].result)).dividedBy(10 ** 18).toString()) || "0";

    const sOlpLastAddedAt = (response[9].result && response[9].result.toString()) || "0";
    const mOlpLastAddedAt = (response[11].result && response[11].result.toString()) || "0";
    const lOlpLastAddedAt = (response[13].result && response[13].result.toString()) || "0";

    const sOlpCooldownDuration = (response[10].result && response[10].result.toString()) || "0";
    const mOlpCooldownDuration = (response[12].result && response[12].result.toString()) || "0";
    const lOlpCooldownDuration = (response[14].result && response[14].result.toString()) || "0";

    data.cooldown.sOlp = new BigNumber(sOlpLastAddedAt).plus(sOlpCooldownDuration).toString();
    data.cooldown.mOlp = new BigNumber(mOlpLastAddedAt).plus(mOlpCooldownDuration).toString();
    data.cooldown.lOlp = new BigNumber(lOlpLastAddedAt).plus(lOlpCooldownDuration).toString();

    if (isBerachain(chain)) {
      data.quoteAsset.HONEY =
        (response[15].result &&
          new BigNumber(String(response[15].result))
            .dividedBy(10 ** QA_TICKER_TO_DECIMAL[chain]["HONEY"])
            .toString()) ||
        "0";
      data.quoteToken.honey =
...
export const getOlpMetrics = async (data: IOlpMetrics, chain: SupportedChains) => {
  try {
    for (let i = 0; i < OLP_KEYS.length; i++) {
      ...
      if (isBerachain(chain)) {
        contracts.push({
          ...vaultContract(OLP_KEYS[i], chain),
          functionName: "tokenWeights",
          args: [getContractAddress("HONEY", chain)],
        });
        contracts.push({
          ...vaultContract(OLP_KEYS[i], chain),
          functionName: "usdgAmounts",
        ...
        contracts.push({
          ...vaultUtilsContract(OLP_KEYS[i], chain),
          functionName: "getBuyUsdgFeeBasisPoints",
          args: [getContractAddress("HONEY", chain), "1000000000000000000"],
        });
        contracts.push({
          ...vaultUtilsContract(OLP_KEYS[i], chain),
          functionName: "getSellUsdgFeeBasisPoints",
      ...
      const totalTargetWeight = (response[0].result && response[0].result.toString()) || "0";
      const btcTargetWeight = (response[1].result && response[1].result.toString()) || "0";
      const ethTargetWeight = (response[2].result && response[2].result.toString()) || "0";
      const usdcTargetWeight = (response[3].result && response[3].result.toString()) || "0";

      const totalCurrentWeight = (response[4].result && response[4].result.toString()) || "0";
      const btcCurrentWeight = (response[5].result && response[5].result.toString()) || "0";
      const ethCurrentWeight = (response[6].result && response[6].result.toString()) || "0";
      const usdcCurrentWeight = (response[7].result && response[7].result.toString()) || "0";
      ...
      data[OLP_KEYS[i]].buyUsdgFee.wbtc = (response[8].result && response[8].result.toString()) || "0";
      data[OLP_KEYS[i]].buyUsdgFee.weth = (response[9].result && response[9].result.toString()) || "0";
      data[OLP_KEYS[i]].buyUsdgFee.usdc = (response[10].result && response[10].result.toString()) || "0";

      data[OLP_KEYS[i]].sellUsdgFee.wbtc = (response[11].result && response[11].result.toString()) || "0";
      data[OLP_KEYS[i]].sellUsdgFee.weth = (response[12].result && response[12].result.toString()) || "0";
      data[OLP_KEYS[i]].sellUsdgFee.usdc = (response[13].result && response[13].result.toString()) || "0";

      if (isBerachain(chain)) {
        const honeyTargetWeight = (response[14].result && response[14].result.toString()) || "0";
        const honeyCurrentWeight = (response[15].result && response[15].result.toString()) || "0";

        if (honeyTargetWeight === "0") {
          data[OLP_KEYS[i]].targetWeight.honey = "0";
        } else {
          data[OLP_KEYS[i]].targetWeight.honey = new BigNumber(honeyTargetWeight)
            .div(totalTargetWeight)
            .multipliedBy(100)
            .toString();
        }

        if (honeyCurrentWeight === "0") {
          data[OLP_KEYS[i]].currentWeight.honey = "0";
        } else {
          data[OLP_KEYS[i]].currentWeight.honey = new BigNumber(honeyCurrentWeight)
            .div(totalCurrentWeight)
            .multipliedBy(100)
            .toString();
        }

        data[OLP_KEYS[i]].buyUsdgFee.honey = (response[16].result && response[16].result.toString()) || "0";
        data[OLP_KEYS[i]].sellUsdgFee.honey = (response[17].result && response[17].result.toString()) || "0";
      }
    }
  } catch (error) {
    console.error(error);
  ...
  try {
    ...
    data.sOlp.price = (response[0].result && response[0].result.toString()) || "0";
    data.mOlp.price = (response[1].result && response[1].result.toString()) || "0";
    data.lOlp.price = (response[2].result && response[2].result.toString()) || "0";

    data.sOlp.totalSupply = (response[3].result && response[3].result.toString()) || "0";
    data.mOlp.totalSupply = (response[4].result && response[4].result.toString()) || "0";
    data.lOlp.totalSupply = (response[5].result && response[5].result.toString()) || "0";
  } catch (error) {
    console.error(error);
  }
...
export const isPositionRequestExecuted = async (
  ...
  if (Number(positionRequest[9]) === 2) {
    ...
    if (isOpen) {
      ...
      data.closedCollateralAmount = new BigNumber(positionRequest[10]).toString();
      data.executionPrice = new BigNumber(positionRequest[11]).toString();
      data.lastProcessBlockTime = new BigNumber(positionRequest[12]).toString();
    }
