...
const SingleAnnouncement: React.FC<SingleAnnouncementProps> = ({ announcement, isCheckable, onClose }) => {
  ...
  const handleHide = (e: React.MouseEvent) => {
    ...
    setHide(!hide);
    if (!hide) {
      localStorage.setItem(`moby:announcementBar:${announcement.id}:hideUntil`, (Date.now() + 24 * 60 * 60 * 1000).toString());
    } else {
      localStorage.removeItem(`moby:announcementBar:${announcement.id}:hideUntil`);
    }
  };
