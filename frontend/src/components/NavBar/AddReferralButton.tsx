...
import IconReferralBlack from "@assets/icon-referral-black.svg";
import IconReferralWhite from "@assets/icon-referral-white.svg";
...
const AddReferralButton: React.FC = () => {
  ...
  // @dev 
  // Open the empty modal when the referral button is clicked to show dimmed background
  useEffect(() => {
    if (isReferralsOpen) {
      openModal(<></>, {
        modalClassName: [
  ...
  useEffect(() => {
    const handleClick = (event: any) => {
      if (referralsRef.current?.contains(event.target)) {
        return;
      } 
    ...
    <div ref={referralsRef} className="relative">
      <div
        className={twJoin(
          ...
          "h-[36px] w-[92px] pl-[8px] pr-[14px] rounded-[4px] bg-black29",
      ...
      > 
        <div className="flex flex-row items-center gap-[8px] text-whitee0">
          <img className="w-[24px]" src={IconReferralWhite}/>
          <p className="text-[15px] font-semibold">Invite</p>
        </div>
      </div>
      {isReferralsOpen && (
          <div className={twJoin(
            ...
            <div className="flex flex-row items-center justify-between h-[24px]">
              ...
              <img className="cursor-pointer w-[24px] h-[24px]" src={IconCloseGreen} onClick={() => {
                if (isReferralsOpen) {
                  closeModal();
                }
                setIsReferralsOpen(!isReferralsOpen)
