...
import { FEE_RATES } from "@moby/shared";
...
const SelectedOptionDetail: React.FC<SelectedOptionDetail> = ({
  ...
  const renderRiskPremiumDiffBadge = () => {
    ...
    if (isComboMode && riskPremium !== 0) {
      ...
      riskPremiumDiffInPercentage =
        (Math.abs(riskPremium - riskPremiumAtComboMode) / riskPremiumAtComboMode) * 100;
    }
  ...
  const executionPrice = isBuy ? markPrice + riskPremium : markPrice - riskPremium;
        ...
        <div className="flex flex-col w-full px-[18px] ">
          <div className="flex flex-row justify-between w-full h-[18px]">
            ...
            <div className="flex flex-row gap-[8px]">
              {isComboMode ? (
                <p className="text-[14px] text-whitee0 font-semibold">
                  {advancedFormatNumber(executionPriceAtComboMode || 0, 2, "$")}
                </p>
              ) : (
                <p className="text-[14px] text-whitee0 font-semibold">
                  {advancedFormatNumber(executionPrice || 0, 2, "$")}
                </p>
              )}
            </div>
          </div>
          <div className="flex flex-col mt-[8px] pb-[12px] h-[38px]">
            <div className="flex flex-row items-center justify-between h-[14px] w-full">
              ...
              <div>
                {isComboMode ? (
                  <p className="text-[12px] text-gray80 font-semibold">
            ...
            <div className="flex flex-row items-center justify-between h-[22px] min-h-[22px] w-full mt-[2px]">
              <div className="flex flex-row items-center gap-[8px]">
                ...
                <WithTooltip
                  tooltipContent={
                    <p className={twJoin("leading-[0.85rem] text-[12px] font-[600]")}>
                      The mechanism that penalizes trades adding to the platform’s Greeks exposure and rewards
                      trades that reduce it.
                    </p>
                  }
                ...
                >
                  <span
                    className={twJoin(
                      "h-[14px] text-[12px] text-gray80 font-semibold leading-3",
        ...
        <div className="flex flex-row justify-between w-full h-[22px] px-[18px] ">
          <div className="relative group cursor-help w-fit h-full font-semibold">
            <div className="flex flex-row items-center h-full gap-[8px]">
              <p className="text-gray80 border-b-[1px] border-dashed border-b-greenc1 hover:text-greenc1 ">
                Trade Fee
              </p>
              {selectedOrderSide === "Buy" && isComboMode && (
                ...
                >
                  <p className="text-[12px] text-whitee0 font-semibold">
                    {(Math.abs(FEE_RATES.OPEN_BUY_NAKED_POSITION - FEE_RATES.OPEN_COMBO_POSITION) /
            ...
            >
              <p className="text-[12px] text-gray80 leading-[0.85rem]">
                Trade Fee is <span className="text-gray98 font-bold">0.03%</span> of the notional volume for
                all trades,
                <br />
                except <span className="text-gray98 font-bold">0.06%</span> for buying naked options, not
                exceeding
                <br />
          ...
          <p className="text-[14px] text-whitee0 font-semibold">
            {advancedFormatNumber(isComboMode ? tradeFeeUsdAtComboMode : tradeFeeUsd || 0, 2, "$")}
          </p>
        </div>
        ...
        {isSlippageDetailsOpen && (
          <div className="flex flex-col mt-[4px] gap-[16px]">
            <div className="flex flex-row justify-between items-center h-fit px-[18px]">
              <div className="flex flex-row justify-between items-center w-[167px] h-[34px] p-[4px] bg-black12 rounded-[6px] border-[1px] border-black12">
                {slippageValues.map((value, index) => (
                  <button
                    key={value}
                    className={twJoin(
                      "w-[48px] h-full text-[12px] text-center rounded-[4px]",
                      slippage === value ? "h-[12px] text-[#E6FC8D] bg-black29 font-bold" : "h-full text-gray80 font-normal"
                    )}
                    onClick={() => {
              ...
              <div className="flex flex-row justify-between items-center w-[125px] h-[34px] px-[12px] py-[4px] rounded-[4px] bg-black29">
                <input
                  ...
                  className={twJoin(
                    ...
                    "focus:outline-none",
                    "placeholder:text-[12px] placeholder:text-gray52 placeholder:font-semibold"
                  )}
                  onChange={(e) => {
            ...
            <div className="flex flex-col px-[18px] gap-[4px]">
              <p className="text-[12px] font-semibold text-greene6">Why this number?</p>
              <p className="text-[12px] text-[#666666] mt-[2px]">
