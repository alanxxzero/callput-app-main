...
import { FuturesAssetIndexMap, SpotAssetIndexMap, UnderlyingAssetWithAll } from "@moby/shared";
...
import { PositionHistoryState, PositionHistoryWithMetadata } from "@/store/slices/PositionHistorySlice";
...
function PositionManagementPanel() {
  const [underlyingAsset, setUnderlyingAsset] = useState<UnderlyingAssetWithAll>("ALL");
  ...
  const positionHistoryData = useAppSelector((state: any) => state.positionHistory) as PositionHistoryState;
  ...
  }, [underlyingAsset, positionHistoryData, selectedHistoryFilterType, selectedHistoryTimestamp, chain]);
  ...
  return (
    <div className="flex flex-col h-fit w-full">
      <div
        className={twJoin(
          "flex flex-row items-center justify-between w-full h-[72px] px-[12px] py-[14px]",
          "border-t-[1px] border-black29"
        )}
      >
