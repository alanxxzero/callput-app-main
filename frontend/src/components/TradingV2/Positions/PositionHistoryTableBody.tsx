...
function PositionHistoryTableBody({ history }: PositionHistoryTableBodyProps) {
  ...
  return (
    <div
      className={twJoin("w-full min-h-[332px] py-[6px] bg-black0a", "border-t-[1px] border-t-[#292929]")}
    >
...
function NoPositions() {
  return (
    <div
      className={twJoin(
        "w-full min-h-[332px] bg-black0a",
        "border-t-[1px] border-t-[#292929]",
        "flex flex-row items-center justify-center"
      )}
    >
      <p className="text-gray80 text-[13px] font-medium leading-[18px]">No trade history</p>
    </div>
  );
}
  ...
  return (
    <div key={index} className={historyTableRowStyles}>
      <div className="w-[128px] min-w-[128px]">
        <p className="w-full h-[16px] font-medium leading-normal">{history.metadata.type}</p>
      ...
      <div className="w-full min-w-[74px] max-w-[126px] text-right">
        <div className="flex flex-row items-center justify-end">
          {(history.metadata.type === "Settle" || history.metadata.type === "Close") && (
            <ShareButton
              shareData={{
                instrument: history.metadata.instrument,
                optionDirection: history.metadata.optionDirection,
...
function CollateralDisplay({
  ...
  if (optionOrderSide === "Buy" || type === "Transfer") {
    return (
      <div className={twJoin("w-full text-grayb3", width.min, width.max, textClass)}>
  ...
  return (
    <div className={twJoin("w-full text-green63", width.min, width.max, valueClass)}>
      <p className={twJoin("w-full")}>{advancedFormatNumber(collateral, 4, "")}</p>
    </div>
  );
}
