...
function OptionPreviewTradeButton({
  ...
  const { isApproved: isQuoteAssetApproved, balance: quoteAssetBalance } = useQuoteAssetInfo(
    quoteAsset,
    orderSide === "Buy" ? quoteAssetAmountForVanilla : quoteAssetAmountForSpread
  );

  const { isApproved: isCollateralAssetForVanillaApproved, balance: collateralAssetBalanceForVanilla } =
    useQuoteAssetInfo(collateralAssetForVanilla, collateralAssetAmountForVanilla);

  const { isApproved: isCollateralAssetForSpreadApproved, balance: collateralAssetBalanceForSpread } =
    useQuoteAssetInfo(collateralAssetForSpread, collateralAssetAmountForSpread);
  ...
  const handleApproveForQuoteAsset = async (
    ...
    const amountToApprove = new BN(amount)
      .multipliedBy(1.5)
      .multipliedBy(
        10 ** QA_TICKER_TO_DECIMAL[chain][quoteAsset as keyof (typeof NetworkQuoteAsset)[typeof chain]]
      )
      .toFixed(0);

    const result = await writeApproveERC20(token, spender, BigInt(amountToApprove));
  ...
  if (orderSide === "Sell" && optionStrategy === "Vanilla" && !isCollateralAssetForVanillaApproved) {
    return (
      <ApproveButton
        asset={collateralAssetForVanilla}
        onClick={() => handleApproveForQuoteAsset(collateralAssetForVanilla, collateralAssetAmountForVanilla)}
      />
    );
  }

  if (orderSide === "Sell" && optionStrategy === "Spread" && !isCollateralAssetForSpreadApproved) {
    return (
      <ApproveButton
        asset={collateralAssetForSpread}
        onClick={() => handleApproveForQuoteAsset(collateralAssetForSpread, collateralAssetAmountForSpread)}
      />
    );
  }

  if (optionStrategy === "Vanilla" ? sizeForVanilla === "0" : sizeForSpread === "0") {
  ...
  if (
    !isTradeAvailable({
      ...
      quoteAssetAmount: orderSide === "Buy" ? quoteAssetAmountForVanilla : quoteAssetAmountForSpread,
      collateralAssetAmount:
        orderSide === "Buy" ? collateralAssetAmountForVanilla : collateralAssetAmountForSpread,
    })
  ) {
  ...
  if (isButtonError) {
    return <ErrorButton name={buttonNameForError} onClick={() => {}} isError={isButtonError} />;
  }
...
const ButtonContainer = ({ children }: { children: React.ReactNode }) => (
  <div className="h-[88px] p-[20px]">{children}</div>
);
...
const ConnectWalletButton = ({ onClick }: { onClick: () => void }) => (
  <ButtonContainer>
    <Button name="Connect Wallet" color="greenc1" onClick={onClick} />
  </ButtonContainer>
);

const ApproveButton = ({ asset, onClick }: { asset: string; onClick: () => void }) => (
  <ButtonContainer>
...
const EnterAmountButton = () => (
  <ButtonContainer>
    <Button name="Enter Amount to Buy" color="default" disabled={true} onClick={() => {}} />
  </ButtonContainer>
);

const TradeUnavailableButton = () => (
  <ButtonContainer>
    <Button name="Trade Unavailable for Given Size" color="default" disabled={true} onClick={() => {}} />
  </ButtonContainer>
);

const ErrorButton = ({ name, onClick, isError }: { name: string; onClick: () => void; isError: boolean }) => (
  <ButtonContainer>
  ...
  const isMaxOpenSizeExceeded =
    optionStrategy === "Vanilla"
      ? Number(sizeForVanilla) >
        UA_TICKER_TO_MAX_OPEN_SIZE_FOR_VANILLA[chain][underlyingAsset as UnderlyingAsset]
      : Number(sizeForSpread) >
        UA_TICKER_TO_MAX_OPEN_SIZE_FOR_SPREAD[chain][underlyingAsset as UnderlyingAsset];
