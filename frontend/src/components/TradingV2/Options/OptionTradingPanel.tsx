...
function OptionTradingPanel() {
  ...
  const [orderSideForSelectedOption, setOrderSideForSelectedOption] = useState<OrderSide | null>(null);
  const [optionStrategyForSelectedOption, setOptionStrategyForSelectedOption] =
    useState<OptionStrategy | null>(null);

  const [selectedUnderlyingAsset, setSelectedUnderlyingAsset] = useState<UnderlyingAsset>(() => {
    const savedUnderlyingAsset = localStorage.getItem("tradingV2:underlyingAsset");
    return savedUnderlyingAsset ? (savedUnderlyingAsset as UnderlyingAsset) : UnderlyingAsset.BTC;
  });
  ...
  const [selectedOptionDirection, setSelectedOptionDirection] = useState<OptionDirection>(() => {
    const savedOptionDirection = localStorage.getItem("tradingV2:optionDirection");
    return savedOptionDirection ? (savedOptionDirection as OptionDirection) : "Call";
  });
  ...
  const [selectedOptionStrategy, setSelectedOptionStrategy] = useState<OptionStrategy>(() => {
    const savedOptionStrategy = localStorage.getItem("tradingV2:optionStrategy");
    return savedOptionStrategy ? (savedOptionStrategy as OptionStrategy) : "Vanilla";
  });
  ...
  const handleOptionSelection = (option: IOptionDetail) => {
    ...
    if (!option.instrument) {
      ...
      setOptionStrategyForSelectedOption(null);
      return;
    }

    setOrderSideForSelectedOption(selectedOrderSide);
    setOptionStrategyForSelectedOption(selectedOptionStrategy);
  };
  ...
  return (
    <div className="flex flex-row h-fit w-full">
      ...
      />
      {selectedOption.instrument && optionStrategyForSelectedOption && orderSideForSelectedOption ? (
        <OptionPreview
          selectedOption={selectedOption}
          orderSideForSelectedOption={orderSideForSelectedOption}
          optionStrategyForSelectedOption={optionStrategyForSelectedOption}
          setOptionStrategyForSelectedOption={setOptionStrategyForSelectedOption}
