...
import { OptionStrategy, OrderSide, PriceUnit, OptionDirection } from "@/utils/types";
...
function OptionChainTableBody({
  ...
  const [extractedOptions, setExtractedOptions] = useState<{ [key: string]: IOptionDetail }>({});
  ...
  useEffect(() => {
    ...
    const maxVolume = Math.max(...selectedOptions.map((option: IOptionDetail) => option.volume));
  ...
  useEffect(() => {
    const fetchMarketDaily = async () => {
      ...
      const extractedOptions = extractOptions(data.data.market) as { [key: string]: IOptionDetail };
  ...
  return (
    <div
      className={twJoin("w-full min-h-[678px] bg-black0a", "py-[6px]", "border-t-[1px] border-t-[#292929]")}
    >
      <div className={twJoin("flex flex-col w-full h-full scrollbar-hide")}>
        {selectedOptions.map((option: IOptionDetail, index: number) => {
          const optionPair = getBestOptionPair(selectedOrderSide, option, selectedOptions);
...
function UnderlyingFuturesRow({ underlyingFutures }: { underlyingFutures: number }) {
  return (
    <div className="relative flex flex-row items-center">
      <div className="absolute w-full h-[1px] bg-black33" />
      <div
        className={twJoin(
          "z-0",
          "flex flex-row justify-center items-center px-[10px] py-[6px]",
          "w-fulll h-[28px] bg-black33 rounded-[4px] ml-[20px]"
        )}
      >
        <span className="text-[12px] text-[#E6FC8D] font-ibm font-medium">
