...
import OptionPreviewStrategySelector from "./OptionPreviewStrategySelector";
...
import { calculateRiskPremiumRate, calculateUnderlyingFutures, FuturesAssetIndexMap, NetworkQuoteAsset, RiskFreeRateCollection, SpotAssetIndexMap, UnderlyingAsset, VolatilityScore } from "@moby/shared";
...
interface OptionPreviewProps {
  ...
  setOptionStrategyForSelectedOption: (optionStrategy: OptionStrategy) => void;
...
function OptionPreview({
  ...
  setOptionStrategyForSelectedOption,
  snapshot,
}: OptionPreviewProps) {
  const { chain } = useAppSelector(state => state.network) as NetworkState;

  const [localSnapshot, setLocalSnapshot] = useState<previewSnapshot>(() => ({ ...snapshot }));
  const { underlyingAsset, expiry, optionDirection } = localSnapshot;

  const [selectedOptionPair, setSelectedOptionPair] = useState<IOptionDetail>(initialOptionDetail);
  ...
  const [basedExecutionPriceForVanilla, setBasedExecutionPriceForVanilla] = useState<number>(0);
  const [basedExecutionPriceForSpread, setBasedExecutionPriceForSpread] = useState<number>(0);
  ...
  const [executionPriceForVanilla, setExecutionPriceForVanilla] = useState<number>(0);
  const [executionPriceForSpread, setExecutionPriceForSpread] = useState<number>(0);

  const [sizeForVanilla, setSizeForVanilla] = useState<string>("0");
  const [sizeForSpread, setSizeForSpread] = useState<string>("0");
  const [availableSizeForVanilla, setAvailableSizeForVanilla] = useState<number>(0);
  const [availableSizeForSpread, setAvailableSizeForSpread] = useState<number>(0);

  const [quoteAsset, setQuoteAsset] = useState<NetworkQuoteAsset<SupportedChains>>(
    getBaseQuoteAsset(underlyingAsset, optionDirection, "Buy", optionStrategyForSelectedOption)
  );
  const [quoteAssetAmountForVanilla, setQuoteAssetAmountForVanilla] = useState<string>("0");
  const [quoteAssetAmountForSpread, setQuoteAssetAmountForSpread] = useState<string>("0");
  const [quoteAssetValueForVanilla, setQuoteAssetValueForVanilla] = useState<number>(0);
  const [quoteAssetValueForSpread, setQuoteAssetValueForSpread] = useState<number>(0);

  const collateralAssetForVanilla = getBaseQuoteAsset(underlyingAsset, optionDirection, "Sell", "Vanilla");
  const collateralAssetForSpread = getBaseQuoteAsset(underlyingAsset, optionDirection, "Sell", "Spread");

  const [collateralAssetAmountForVanilla, setCollateralAssetAmountForVanilla] = useState<string>("0");
  const [collateralAssetAmountForSpread, setCollateralAssetAmountForSpread] = useState<string>("0");
  ...
  const olpStats = useAppSelector((state: any) => state.market.olpStats) as IOlpStats;
  ...
  const handleCalculateRiskPremium = (size: string, isBuy: boolean, isVanilla: boolean) => {
  ...
  useEffect(() => {
    const { markPrice, basedExecutionPrice } = calculateMarkPriceAndBasedExecutionPrice(
      selectedOption,
      orderSideForSelectedOption
    );
  ...
  useEffect(() => {
    ...
    const markPrice = Math.max(selectedOption.markPrice - selectedOptionPair.markPrice, 0);
    ...
    const basedExecutionPrice =
      orderSideForSelectedOption === "Buy" ? markPrice + basedRiskPremium : markPrice - basedRiskPremium;
  ...
  useEffect(() => {
    ...
    const savedUsdcAmountForClose = new BN(olpAssetAmounts.usdc.depositedAmount)
      .multipliedBy(0.03)
      .toNumber();
    const parsedOlpUsdcAvailableAmounts = new BN(olpAssetAmounts.usdc.availableAmount)
  ...
  return (
    <div
      className={twJoin(
        ...
        "bg-black0a border-l-[1px] border-[#333333]"
      )}
    >
      <OptionPreviewStrategySelector
        selectedOption={selectedOption}
        underlyingAsset={underlyingAsset}
        optionDirection={optionDirection}
        orderSide={orderSideForSelectedOption}
        optionStrategy={optionStrategyForSelectedOption}
        setOptionStrategy={setOptionStrategyForSelectedOption}
      ...
      <OptionPreviewTradeSummary
        ...
        collateralAsset={
          optionStrategyForSelectedOption === "Vanilla" ? collateralAssetForVanilla : collateralAssetForSpread
        }
        ...
        size={optionStrategyForSelectedOption === "Vanilla" ? sizeForVanilla : sizeForSpread}
        executionPrice={
          optionStrategyForSelectedOption === "Vanilla" ? executionPriceForVanilla : executionPriceForSpread
        }
        slippage={slippage}
...
function calculateMarkPriceAndBasedExecutionPrice(selectedOption: IOptionDetail, orderSide: OrderSide) {
...
function getStrategyByOptionTokenId(isVanilla: boolean, isBuy: boolean, isCall: boolean): Strategy {
  if (isVanilla) {
    if (isBuy) {
...
function calculateAvailableSize(
  ...
  if (isBuy) {
    if (isVanilla && isCall) {
      ...
      const strikePriceAmounts = new BN(strikePrice).dividedBy(usdcSpotIndex).toNumber();
      return new BN(parsedOlpUsdcAvailableAmounts).div(strikePriceAmounts).toNumber();
    }
  }

  // Sell orders (both call and put)
  const appliedExecutionPrice = executionPrice === 0 ? basedExecutionPrice : executionPrice;
  const appliedExecutionPriceAmounts = new BN(appliedExecutionPrice).dividedBy(usdcSpotIndex).toNumber();

  return appliedExecutionPriceAmounts === 0
    ? 0
    : new BN(parsedOlpUsdcAvailableAmounts).div(appliedExecutionPriceAmounts).toNumber();
}
