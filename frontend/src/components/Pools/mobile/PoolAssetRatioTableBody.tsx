...
const PoolAssetRatioTableBody: React.FC<PoolAssetRatioTableBodyProps> = ({
  ...
  const olpAssetList = QA_LIST[chain].filter((asset) => {
    ...
    if (!isBerachain && asset === "HONEY") return false;
    return true;
  })
    ...
    <div className="py-3 flex flex-col gap-y-3">
      {olpAssetList.map((token, index) => {
        ...
        if (token === "WBTC") {
          ...
          targetTokenWeight = olpMetrics.targetWeight.honey;
          currentTokenWeight = olpMetrics.currentWeight.honey;
          buyUsdgFee = olpMetrics.buyUsdgFee.honey;
          sellUsdgFee = olpMetrics.sellUsdgFee.honey;
          depositedValue = olpDepositedAssetValue.honey;
        }
        ...
        return (
          <div key={index} className={twJoin("flex flex-row justify-between")}>
            ...
            <div className="flex flex-row gap-x-5">
              ...
              <p
                className={twJoin(
                  "font-semibold font-plex-mono text-greene6",
                  "text-[11px] leading-[16px] md:text-[13px]"
                )}
              >
                {isBuy
