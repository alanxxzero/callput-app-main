...
import { ILeadTraders } from "@/interfaces/interfaces.marketSlice";
import { ModalContext } from "@/components/Common/ModalContext";
import { getUserCopyTradeData } from "@/components/NavBar/AccountInfo";
  ...
  return (
    <div className="px-3 text-[14px] md:text-[16px] leading-[21px] font-semibold">
      ...
      >
        ...
        <p className="text-right font-plex-mono text-greene6">{total}</p>
      </div>
      ...
      {isOpen && (
        <div className="pt-3 pl-[26px] text-gray9D">
          ...
          <div className="flex justify-between items-center gap-x-2 mb-3">
            ...
            <p className="text-right font-plex-mono">{btc}</p>
          </div>
          ...
          <div className="flex justify-between items-center gap-x-2">
            ...
            <p className="text-right font-plex-mono">{eth}</p>
          </div>
        </div>
      )}
    </div>
      ...
      <div className="flex-1 overflow-auto">
        ...
        <div
          className={twJoin(
            ...
            "text-[14px] md:text-[16px] leading-[21px] font-semibold",
            "backdrop-blur-[20px] bg-[rgba(17,22,19,0.85)]"
          )}
        >
          {/* Total Copy Traders */}
          <div className="flex justify-between px-3">
            <p className="text-whitef0">Copy Traders</p>
            <p className="text-right font-plex-mono text-greene6">
              {advancedFormatNumber(copyTraders, 0)}
            </p>
          </div>

          {/* Total Copy Trades Volume */}
          <div className="flex justify-between px-3">
            <p className="text-whitef0">Copy Trades Volume</p>
            <p className="text-right font-plex-mono text-greene6">
          ...
          <div className="flex justify-between px-3">
            <p className="text-whitef0">Rebates from Copy Trades</p>
            <p className="text-right font-plex-mono text-greene6">
              {advancedFormatNumber(Number(rebatesFromCopyTrades), 2, "$")}
            </p>
          </div>
        </div>
      </div>
