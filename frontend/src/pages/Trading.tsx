...
import { calculateUnderlyingFutures, FuturesAssetIndexMap, NormalizedFuturesAsset, RiskFreeRateCollection, UnderlyingAsset } from "@moby/shared";
...
function Trading({ announcementsLen }: TradingProps) {
  ...
  const [selectedUnderlyingAsset, setSelectedUnderlyingAsset] = useState<UnderlyingAsset>(() => {
    const savedUnderlyingAsset = localStorage.getItem("selectedUnderlyingAsset");
  ...
  const [selectedOption, setSelectedOption] = useState<IOptionDetail>(initialOptionDetail);
  ...
  }, [selectedExpiry, selectedUnderlyingAsset, selectedOptionDirection, selectedOrderSide]);
  ...
  useEffect(() => {
    ...
    if (selectedExpiry !== 0) {
      ...
      const filteredSelectedMarket = marketToSelect.filter((option: any) => option.isOptionAvailable);
  ...
  useEffect(() => {
    ...
    let closestIndex = selectedMarket.findIndex((option) => Number(option.strikePrice) >= underlyingFutures);

    setFuturesPriceIndex(closestIndex);

    const rowHeight = 44; // adjust this to your actual row height

    if (tableContainerRef.current && !shouldDisableTrade && !hasInitialScroll && closestIndex !== 0) {
      const halfTableHeight = tableContainerRef.current.offsetHeight / 2 - 132;
      tableContainerRef.current.scrollTop = closestIndex * rowHeight - halfTableHeight;
  ...
  const shouldDisableTrade =
    new Date().getTime() / 1000 >= new Date(selectedExpiry * 1000).getTime() / 1000 - 1800;
  ...
  // Strike Price Pair
  const [selectableOptionPairs, setSelectableOptionPairs] = useState<IOptionDetail[]>([]);
  ...
  useEffect(() => {
    ...
    selectedMarket.map((option) => {
      ...
      if (
        (selectedOptionDirection === "Call" && option.strikePrice > selectedStrikePrice) ||
        (selectedOptionDirection === "Put" && option.strikePrice < selectedStrikePrice)
      ) {
        const pairedOptionMarkPrice = option.markPrice;

        if (selectedOrderSide === "Buy") {
          const diffInMarkPrice = Math.abs(selectedMarkPrice - pairedOptionMarkPrice);
          if (diffInMarkPrice <= MIN_MARK_PRICE_FOR_BUY_POSITION[selectedUnderlyingAsset]) return;
        }
  ...
  return (
    ...
    >
      <div className={twJoin("w-[1280px] max-w-[1280px] min-w-[1280px] min-h-screen pt-[64px]")}>
        <div className={twJoin("w-full h-full", "flex flex-col gap-[48px]")}>
          <div className={twJoin("w-full", "flex flex-row justify-between gap-[24px]")}>
            {/* Trade 페이지 상단 왼쪽 영역 */}
            <div className={twJoin("flex flex-col gap-[48px]", "w-[856px] min-h-[790px]")}>
              {/* Trade 페이지 상단 왼쪽 영역 - Dashboard */}
              <div
                className={twJoin("w-full h-[153px] max-h-[153px] min-h-[153px]", "flex flex-col gap-[21px]")}
              >
                <ToggleAsset
                    ...
                    {selectedMarket.length > 0 ? (
                      <>
                        {shouldDisableTrade && (
                          <div className="absolute flex flex-row justify-center items-center w-full h-full text-center leading-[1.25rem]">
                            ...
                            positions with options expiring within 30 minutes <br />
                            cannot be opened or closed. <br />
                          </div>
                        )}
                        {selectedMarket.map((option, index) => {
                          const shouldShowAssetPrice = index === futuresPriceIndex && underlyingFutures !== 0;
                          const isFuturesPriceIndexOutOfRange =
                            futuresPriceIndex === -1 && index === selectedMarket.length - 1;
                          ...
                          return (
                            <TradeOptionsTableBody
                              ...
                              isFuturesPriceIndexOutOfRange={isFuturesPriceIndexOutOfRange}
                              selectedUnderlyingAsset={selectedUnderlyingAsset}
                      ...
                      <div className="relative flex flex-row justify-center items-center w-full h-full text-[13px] text-gray52 font-semibold">
                        ...
                        >
                          ...
                          <span className="text-greenc1">${formatNumber(futuresIndex, 2, true)}</span>
                        </div>
          ...
          <div className="flex flex-col gap-[22px]">
            ...
            {selectedTradeDataMenuType === "My Positions" && (
              ...
              >
                <MyPositionTableHead />
                <div ref={positionTableRef} className="w-full h-[2px] bg-black29" />
            ...
            {selectedTradeDataMenuType === "History" && (
              <div>
                ...
                <div className="flex justify-end w-full mt-[16px]">
                  ...
                  >
                    Last updated {timeSince(positionHistoryData.lastUpdatedAt)} ago
                  </div>
                </div>
              </div>
            )}
