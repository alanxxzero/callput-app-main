...
const processTopGainerOptions = (
  ...
  const topGainerOptionsData: TopGainerOption[] = [];
...
const processHighestVolumeOptions = (
  currentMarketDataArb: Map<string, { markPrice: number; volume: number }>,
  currentMarketDataBera: Map<string, { markPrice: number; volume: number }>
) => {
  const highestVolumeOptionsData: HighestVolumeOption[] = [];

  currentMarketDataArb.forEach((data, instrument) => {
    if (data.volume > 0) {
  ...
  currentMarketDataBera.forEach((data, instrument) => {
    if (data.volume > 0) {
      const existingOption = highestVolumeOptionsData.find((option) => option.instrument === instrument);

      if (existingOption) {
        existingOption.volume += data.volume;
      } else {
        highestVolumeOptionsData.push({
          instrument,
          currentPrice: data.markPrice,
...
export const LeaderBoardProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [data, setData] = useState<LeaderBoardData>(LEADER_BOARD_DATA);
  ...
  const refetch = async () => {
    try {
      ...
      const [currentMarketDataArb, dailyMarketDataArb, currentMarketDataBera] = await Promise.all([
        // Arb
        fetch(API_URLS.MARKET_DATA_ARB).then((res) => res.json() as Promise<MarketData>),
        fetch(`${API_URLS.MARKET_DAILY_ARB}-${dateISOString}.json`).then(
          (res) => res.json() as Promise<MarketData>
        ),

        // Bera
        fetch(API_URLS.MARKET_DATA_BERA).then((res) => res.json() as Promise<MarketData>),
      ]);

      const extractedCurrentMarketDataArb = extractOptions(currentMarketDataArb);
      const extractedDailyMarketDataArb = extractOptions(dailyMarketDataArb);
      const extractedCurrentMarketDataBera = extractOptions(currentMarketDataBera);

      const topGainerOptions = processTopGainerOptions(
        extractedCurrentMarketDataArb,
        extractedDailyMarketDataArb
      );
      const highestVolumeOptions = processHighestVolumeOptions(
        extractedCurrentMarketDataArb,
        extractedCurrentMarketDataBera
      );

      setData({
        ...data,
        topGainerOptions: topGainerOptions,
  ...
  return (
    <LeaderBoardContext.Provider value={{ data, isLoading, error, refetch }}>
      {children}
