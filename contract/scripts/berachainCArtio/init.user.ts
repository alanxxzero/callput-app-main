...
export const initUser = async (ethers: any, addressMap: any) => {
  ...
  await molp.connect(user1).approve(CONTRACT_ADDRESS.M_REWARD_TRACKER, ethers.MaxUint256)
  await lolp.connect(user1).approve(CONTRACT_ADDRESS.L_REWARD_TRACKER, ethers.MaxUint256)

  await usdc.connect(user1).approve(CONTRACT_ADDRESS.S_OLP_MANAGER, ethers.MaxUint256)
  await usdc.connect(user1).approve(CONTRACT_ADDRESS.M_OLP_MANAGER, ethers.MaxUint256)
  await usdc.connect(user1).approve(CONTRACT_ADDRESS.L_OLP_MANAGER, ethers.MaxUint256)

  await wbtc.connect(user1).approve(CONTRACT_ADDRESS.S_OLP_MANAGER, ethers.MaxUint256)
  await wbtc.connect(user1).approve(CONTRACT_ADDRESS.M_OLP_MANAGER, ethers.MaxUint256)
  await wbtc.connect(user1).approve(CONTRACT_ADDRESS.L_OLP_MANAGER, ethers.MaxUint256)
  ...
  await molp.connect(user2).approve(CONTRACT_ADDRESS.M_REWARD_TRACKER, ethers.MaxUint256)
  await lolp.connect(user2).approve(CONTRACT_ADDRESS.L_REWARD_TRACKER, ethers.MaxUint256)
  
  await usdc.connect(user2).approve(CONTRACT_ADDRESS.S_OLP_MANAGER, ethers.MaxUint256)
  await usdc.connect(user2).approve(CONTRACT_ADDRESS.M_OLP_MANAGER, ethers.MaxUint256)
  await usdc.connect(user2).approve(CONTRACT_ADDRESS.L_OLP_MANAGER, ethers.MaxUint256)
  
  await wbtc.connect(user2).approve(CONTRACT_ADDRESS.S_OLP_MANAGER, ethers.MaxUint256)
  await wbtc.connect(user2).approve(CONTRACT_ADDRESS.M_OLP_MANAGER, ethers.MaxUint256)
  await wbtc.connect(user2).approve(CONTRACT_ADDRESS.L_OLP_MANAGER, ethers.MaxUint256)
}