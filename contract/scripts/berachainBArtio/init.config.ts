...
import { btcMaxUsdgAmount, btcTokenWeight, ethMaxUsdgAmount, ethTokenWeight, honeyBufferAmounts, honeyMaxUsdgAmount, honeyTokenWeight, mpReleaseDuration, referralDiscountRate, referralFeeRebateRate, rpReleaseDuration, settleFeeCalculationLimitRate, socialTradingLeaderFeeRebateRate, tradeFeeCalculationLimitRate, usdcBufferAmounts, usdcMaxUsdgAmount, usdcTokenWeight, wbtcBufferAmounts, wethBufferAmounts } from "./constants";
...
export const init = async (ethers: any, addressMap: any) => {
  ...
  for (let i = 0; i < vaults.length; i++) {
    ...
    await vaults[i].connect(DEPLOYER).setTokenConfig(CONTRACT_ADDRESS.HONEY, honeyDecimal, honeyTokenWeight, honeyMaxUsdgAmount, false, true)
