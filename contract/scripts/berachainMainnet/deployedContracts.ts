...
export const getDeployedContracts = async (ethers: any, addressMap: any) => {
  const contractAddress = addressMap || CONTRACT_ADDRESS
  ...
  switch (process.env.HARDHAT_NETWORK) {
    ...
    case 'berachainBArtio':
      chainPrimaryOracle = 'BerachainBArtioPrimaryOracle'
      break
    case 'berachainCArtio':
      chainPrimaryOracle = 'BerachainCArtioPrimaryOracle'
      break
    case 'newArbitrumOne':
      chainPrimaryOracle = 'ArbitrumPrimaryOracle'
      break
    case 'berachainMainnet':
      chainPrimaryOracle = 'BerachainMainnetPrimaryOracle'
      break
    default:
  ...
  return {
    ...
    wbera: await ethers.getContractAt("ERC20", contractAddress.WBERA),
    honey: await ethers.getContractAt("ERC20", contractAddress.HONEY),
    ...
    primaryOracle: await ethers.getContractAt(chainPrimaryOracle, contractAddress.PRIMARY_ORACLE),
