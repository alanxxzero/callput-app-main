...
export async function resume(ethers: any, addressMap: any) {
  const [
    DEPLOYER,
  ...
  const tx1 = await sVault.setIsPositionEnabled(true);
  await tx1.wait();
  const tx2 = await sVault.setIsBuySellSwapEnabled(true);
  await tx2.wait();
  const tx3 = await mVault.setIsPositionEnabled(true);
  await tx3.wait();
  const tx4 = await mVault.setIsBuySellSwapEnabled(true);
  await tx4.wait();
