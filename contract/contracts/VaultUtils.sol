...
contract VaultUtils is IVaultUtils, OwnableUpgradeable, AuthorityUtil {
    ...
    // Fee Basis Points
    uint256 public constant OPEN_BUY_NAKED_POSITION_FEE = 6; // 0.06%
    uint256 public constant OPEN_SELL_NAKED_POSITION_FEE = 3; // 0.03%
    uint256 public constant OPEN_COMBO_POSITION_FEE = 3; // 0.03%
    uint256 public constant CLOSE_NAKED_POSITION_FEE = 3; // 0.03%
    uint256 public constant CLOSE_COMBO_POSITION_FEE = 3; // 0.03%
    uint256 public constant SETTLE_POSITION_FEE = 2; // 0.02%
    uint256 public constant TAX_FEE = 60; // 0.6%
    uint256 public constant STABLE_TAX_FEE = 5; // 0.05%
    uint256 public constant MINT_BURN_FEE = 25; // 0.25%
    uint256 public constant SWAP_FEE = 25; // 0.25%
    uint256 public constant STABLE_SWAP_FEE = 1; // 0.01%
    ...
    function initialize(
        ...
        require(_vault != address(0), "VaultUtils: invalid address");
    ...
    modifier onlyVault() {
        require(msg.sender == vault, "VaultUtils: forbidden");
        _;
    }

    function setPositionManager(address _positionManager) external onlyAdmin {
        require(_positionManager != address(0), "VaultUtils: invalid PositionManager");
    ...
    function setTradeFeeCalculationLimitRate(uint256 _tradeFeeCalculationLimitRate) external onlyAdmin {
        require(_tradeFeeCalculationLimitRate <= BASIS_POINTS_DIVISOR, "VaultUtils: invalid trade fee calculation limit rate");
    ...
    function setSettleFeeCalculationLimitRate(uint256 _settleFeeCalculationLimitRate) external onlyAdmin {
        require(_settleFeeCalculationLimitRate <= BASIS_POINTS_DIVISOR, "VaultUtils: invalid settle fee calculation limit rate");
    ...
    function setReferral(address _referral) external onlyAdmin {
        require(_referral != address(0), "VaultUtils: invalid referral");
        referral = _referral;
    ...
    function getPositionFeeBasisPoints(address _account, Utils.Strategy _strategy, bool _isOpen, bool _isSettle) public override view returns (uint256, uint256, address, uint256) {
        require(_account != address(0), "VaultUtils: invalid address for get feeBps and discount rate");
    ...
    function updateVaultPositionAfterTrade(uint256 _expiry, uint256 _optionTokenId, uint256 _size, bool _isAdd) external override onlyController {
        ...
        for (uint256 i = 0; i < tokens.length;) {
            ...
            if (token.optionTokenId == _optionTokenId) {
                if (_isAdd) {
                    ...
                    require(token.size >= _size, "VaultUtils: insufficient optionToken");
                    unchecked {
                        token.size -= _size;
                    }
                }
        ...
        if (_isAdd) {
            ...
            revert("VaultUtils: optionToken not found");
        }
    }
    ...
    function settleSelfOriginPositions(uint256 _expiry, uint256 _endLength) external onlyKeeper returns (bool) {
        require(!isPositionAtSelfExpirySettled[_expiry], "Vault: position already settled");
    ...
    function _removeExpiry(uint256 _expiry, bool _isSettle) internal {
        ...
        require(index < targetArray.length && targetArray[index] == _expiry, "VaultUtils: Expiry not found");
