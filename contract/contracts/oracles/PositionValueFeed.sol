...
contract PositionValueFeed is IPositionValueFeed, OwnableUpgradeable, AuthorityUtil {
    ...
    uint256 public ackPositionKeysStart; // Acknowledged position keys start index
    ...
    function setPositionManager(address _positionManager) external onlyAdmin {
        require(_positionManager != address(0), "PositionValueFeed: invalid position manager");
    ...
    function feedPV(
        ...
        require(positionManager != address(0), "PositionValueFeed: position manager not set");
    ...
    function feedPV_V2(
        ...
        require(block.timestamp <= _deadline, "PositionValueFeed: DEADLINE_EXPIRED");
        require(positionManager != address(0), "PositionValueFeed: position manager not set");
        require(_positionKeysStart == IPositionManager(positionManager).positionRequestKeysStart(), 
                "PositionValueFeed: invalid position keys start");

        pvLastUpdatedAt = block.timestamp;
        ackPositionKeysStart = _positionKeysStart; // Update acknowledged position keys start
    ...
    function getPVAndSign(address _vault) public override view returns (uint256, bool) {
        ...
        require(positionManager != address(0), "PositionValueFeed: position manager not set");
        require(IPositionManager(positionManager).positionRequestKeysStart() == ackPositionKeysStart, 
                "PositionValueFeed: position state mismatch");
