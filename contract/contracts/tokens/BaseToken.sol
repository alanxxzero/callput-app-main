...
contract BaseToken is IERC20, IBaseToken, Initializable, AuthorityUtil {
    ...
    event SetInPrivateTransferMode(bool indexed inPrivateTransferMode);
    ...
    function withdrawToken(address _token, address _account, uint256 _amount) external override onlyAdmin {
        IERC20(_token).safeTransfer(_account, _amount);
    }

    function setInPrivateTransferMode(bool _inPrivateTransferMode) external override onlyAdmin {
    ...
    function transferFrom(address _sender, address _recipient, uint256 _amount) external override returns (bool) {
        ...
        require(currentAllowance >= _amount, "BaseToken: transfer amount exceeds allowance");
    ...
    function _burn(address _account, uint256 _amount) internal {
        ...
        require(currentBalance >= _amount, "BaseToken: burn amount exceeds balance");

        unchecked {
            balances[_account] = currentBalance - _amount;
        }
    ...
    function _transfer(address _sender, address _recipient, uint256 _amount) private {
        ...
        uint256 currentBalance = balances[_sender];
        require(currentBalance >= _amount, "BaseToken: transfer amount exceeds balance");

        unchecked {
            balances[_sender] = currentBalance - _amount;
        }
