...
contract OlpManager is IOlpManager, OwnableUpgradeable, ReentrancyGuardUpgradeable, AuthorityUtil {
    ...
    mapping (address => bool) public isHandler; // handler = RewardRouterV2 for add and remove liquidity, RewardTracker for set last added at
    ...
    function setLastAddedAt(address _account, uint256 _timestamp) external override {
        _validateHandler();
        lastAddedAt[_account] = _timestamp;
    }
    ...
    function getOlpMpRpReleaseRate(address _token) public view returns (uint256, uint256) {
        return (
            block.timestamp < vaultUtils.periodFinish(IVaultUtils.PriceType.MP, _token) 
                ? vaultUtils.releaseRate(IVaultUtils.PriceType.MP, _token) 
                : 0,
            block.timestamp < vaultUtils.periodFinish(IVaultUtils.PriceType.RP, _token)
                ? vaultUtils.releaseRate(IVaultUtils.PriceType.RP, _token)
                : 0
        );
    }
