...
contract Vault is IVault, IERC1155Receiver, OwnableUpgradeable, ReentrancyGuardUpgradeable, AuthorityUtil {
    ...
    function clearTokenConfig(address _token) external onlyAdmin {
        require(isWhitelistedToken[_token], "Vault: token not whitelisted");
    ...
    function directPoolDeposit(address _token) external override nonReentrant {
        ...
        require(tokenAmount > 0, "Vault: invalid token amount");
    ...
    function updatePendingAmount(address _token) public override {
        ...
        require((pendingMpAmounts[_token] >= releaseMpAmount) && (pendingRpAmounts[_token] >= releaseRpAmount), "Vault: releaseAmount exceeded");
    ...
    function getMainStableAsset() public override view returns (address) {
        ...
        require(isWhitelistedToken[mainStableAsset] && isStableToken[mainStableAsset], "Vault: invalid asset");
        require(decimal == tokenDecimals[mainStableAsset], "Vault: invalid asset decimal");
    ...
    function getUnderlyingAssetByIndex(uint16 _underlyingAssetIndex) public override view returns (address) {
        ...
        require(isWhitelistedToken[underlyingAsset] && isUnderlyingAssetToken[underlyingAsset], "Vault: invalid asset");
        require(decimal == tokenDecimals[underlyingAsset], "Vault: invalid asset decimal");
    ...
    function buyUSDG(address _token, address _receiver) external override nonReentrant returns (uint256) {
        ...
        require(isBuySellSwapEnabled, "Vault: buy, sell, and swap not enabled");
        require(isWhitelistedToken[_token], "Vault: token not whitelisted");
        ...
        require(usdgAmount > 0, "Vault: invalid usdg amount");
    ...
    function swap(address _tokenIn, address _tokenOut, address _receiver) external override nonReentrant returns (uint256) {
        ...
        require(isBuySellSwapEnabled, "Vault: buy, sell, and swap not enabled");
        require(isWhitelistedToken[_tokenIn] && isWhitelistedToken[_tokenOut], "Vault: token not whitelisted");
        require(_tokenIn != _tokenOut, "Vault: invalid tokens");

        uint256 amountIn = _transferIn(_tokenIn);
        require(amountIn > 0, "Vault: invalid amount in");
        ...
        require(poolAmounts[_tokenOut] >= bufferAmounts[_tokenOut], "Vault: buffer exceeded");
    ...
    function openPosition(
        ...
        require(isPositionEnabled, "Vault: position not enabled");
        ...
        if (!isBuy) require(markPrice > riskPremium, "Vault: invalid mark price");
    ...
    function clearPosition(
        ...
        require(!Utils.isBuy(_strategy), "Vault: strategy is not sell");
    ...
    function settleVaultPosition(
        ...
        require(msg.sender == address(vaultUtils), "Vault: forbidden");
    ...
    function _calculateEstimatedSizeForOpenPosition(
        ...
        require(estimatedSize > 0, "Vault: invalid estimated size");
    }
    ...
    function _increasePoolAmount(address _token, uint256 _amount) private {
        ...
        require(poolAmounts[_token] <= balance, "Vault: poolAmount exceeded");
        emit IncreasePoolAmount(_token, _amount);
    }

    function _decreasePoolAmount(address _token, uint256 _amount) private {
        poolAmounts[_token] = poolAmounts[_token] - _amount;
        require(reservedAmounts[_token] + pendingMpAmounts[_token] + pendingRpAmounts[_token] <= poolAmounts[_token], "Vault: availableAmounts exceeded)");
    ...
    function _increaseUsdgAmount(address _token, uint256 _amount) private {
        ...
        if (maxUsdgAmount != 0) {
            require(usdgAmounts[_token] <= maxUsdgAmount, "Vault: usdgAmounts exceeded");
        }
    ...
    function _decreaseReservedAmount(address _token, uint256 _amount) private {
        ...
        require(currentAmount + buffer >= _amount, "Vault: buffer exceeded");
    ...
    function _increaseUtilizedAmount(address _token, uint256 _amount) private {
        ...
        require(utilizedAmounts[_token] <= reservedAmounts[_token], "Vault: utilizedAmount exceeded");
    ...
    function _increasePendingAmount(IVaultUtils.PriceType _priceType, address _token, uint256 _pendingAmount) internal {
        require(poolAmounts[_token] - reservedAmounts[_token] - pendingMpAmounts[_token] - pendingRpAmounts[_token] >= _pendingAmount, "Vault: availableAmounts exceeded");
